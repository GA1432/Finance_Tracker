...
```ts
/**
 * src/auth/dto/auth.dto.ts - Authentication Request/Response DTOs
 * 
 * Defines strict validation rules for auth endpoints.
 * Uses class-validator decorators to auto-transform & validate incoming payloads.
 */

import { IsEmail, IsNotEmpty, IsString, MinLength } from 'class-validator';

/**
 * 📥 SignUpDto: Validates new user registration data
 * - Strips/forbids unknown properties via Global ValidationPipe
 */
...
/**
 * 📥 SignInDto: Validates login credentials
 * - Reuses email validation for consistency
 */
...
/**
 * 📤 AuthResponse: Standardized auth endpoint return shape
 * - Omits password from response payload (security)
 * - Returns JWT for subsequent authenticated requests
 */
...
/**
 * 🔒 SafeUser: User type with sensitive fields stripped
 * - Used in controllers/services to prevent accidental password leaks
 */
...
```ts
/**
 * src/auth/auth.module.ts - Auth Feature Module
 * 
 * Wires JWT configuration, Passport strategy, guards, and auth services.
 * Exports JwtAuthGuard so other modules can easily protect routes.
 */
...
@Module({
  imports: [
    // 📦 PrismaModule: Needed for user lookup in signUp/signIn & strategy validation
    PrismaModule,

    // 🔐 PassportModule: Enables @UseGuards() and strategy registration
    PassportModule.register({ defaultStrategy: 'jwt' }),

    // 🎫 JwtModule: Async config pulls secrets from environment (fail-fast)
    JwtModule.registerAsync({
      ...
      useFactory: (config: ConfigService) => ({
        ...
        signOptions: {
          ...
          algorithm: 'HS256', // Explicit symmetric algorithm
        },
      }),
    }),
  ],
  ...
  // 📤 Exports: Makes JwtAuthGuard available to other modules (e.g., ExpenseModule)
 ...
 * 
 * Handles user registration, login, password verification, and JWT generation.
 * Implements secure hashing, atomic DB operations, and fail-fast error handling.
 */
...
import { SignUpDto, SignInDto, AuthResponse, SafeUser } from './dto/auth.dto';
...
export class AuthService {
   ...
   * 1. Checks for existing email (prevents duplicates)
   * 2. Hashes password with bcrypt (salt rounds: 12 for production)
   * 3. Creates user in DB
   * 4. Generates JWT and returns safe response
   */
  async signUp(dto: SignUpDto): Promise<AuthResponse> {
    ...
    // 🔐 Hash password: 12 rounds balances security & performance
   ...
   * 2. Compares provided password with hashed DB value
   * 3. Returns JWT + safe user data
   */
  ...
  /**
   * 🔄 Build Auth Response: Centralized JWT generation & user sanitization
   * - Signs payload with user ID & email
   * - Strips password before returning
   * - Uses environment-configured expiration
   */
 ...
 * 
 * Wraps Passport's AuthGuard('jwt') to enforce JWT validation on protected endpoints.
...
export class JwtAuthGuard extends AuthGuard('jwt') {
  /**
   * 🛡️ canActivate(): Intercepts request before controller method executes
   * - Delegates to Passport for token verification
   * - If valid, attaches req.user and proceeds
   * - If invalid, throws 401 automatically
   */
  ...
  /**
   * 📝 Optional: Custom error response formatting
   * - Uncomment if you want consistent NestJS error shapes instead of Passport's defaults
   */
 ...
 * 
 * Intercepts protected routes, extracts & validates JWT from Authorization header.
 * On success, attaches decoded payload to req.user for downstream use.
 */
...
/**
 * 📦 JwtPayload: Shape of the decoded JWT token
 * - sub: user ID (standard JWT claim)
 * - email: included for convenience in controllers
 */
...
export class JwtStrategy extends PassportStrategy(Strategy) {
  constructor(
    ...
    super({
      ...
      secretOrKey: config.getOrThrow<string>('JWT_SECRET'), // 🔑 Fail if secret missing
      // Optional: Add audience/issuer validation for stricter security
      audience: 'finance-tracker-client',
      issuer: 'finance-tracker-api',
    });
  }
  ...
  /**
   * ✅ validate(): Called automatically after JWT is successfully verified
   * - Receives decoded payload
   * - Can attach additional DB data (e.g., roles, status) to req.user
   * - Returning an object makes it available as req.user in controllers
   */
  async validate(payload: JwtPayload) {
    ...
    // Return minimal user object for route handlers
...
```ts
/**
 * src/prisma/prisma.module.ts - Prisma Feature Module
 * 
 * Wraps PrismaService in a dedicated NestJS module.
 * - exports: [PrismaService] → Makes it injectable across all feature modules
 * - No imports required since PrismaClient is self-contained
 */
...
@Module({
  // ⚙️ Providers: Register PrismaService as a NestJS injectable
  providers: [PrismaService],
  
  // 📤 Exports: Allow other modules (User, Expense, Auth) to inject PrismaService
...
```ts
/**
 * src/prisma/prisma.service.ts - Database Access Layer
 * 
 * Extends PrismaClient to integrate with NestJS lifecycle hooks.
 * Ensures explicit connection on startup, graceful disconnection on shutdown,
 * and provides a centralized place for DB-related diagnostics.
 */
...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  // 📝 Logger instance scoped to this service for clear console/structured logs
  private readonly logger = new Logger(PrismaService.name);

  /**
   * 🟢 OnModuleInit: Runs when NestJS finishes instantiating this provider
   * - Explicitly calls $connect() to fail fast if DATABASE_URL is invalid or DB is down
   * - Prevents silent runtime errors on first query
   */
  async onModuleInit() {
    try {
      await this.$connect();
      ...
      // Re-throw to halt app startup (critical for production readiness)
      throw error;
    }
  }

  /**
   * 🔴 OnModuleDestroy: Runs when app receives SIGTERM/SIGINT or calls app.close()
   * - Ensures all pending queries finish and connection pool drains cleanly
   * - Prevents "ECONNRESET" or orphaned connections during deployments
   */
  async onModuleDestroy() {
    this.logger.log('🔌 Gracefully disconnecting from the database...');
    await this.$disconnect();
    this.logger.log('✅ Database connection closed');
  }

  /**
   * 🛡️ Optional: Centralized Prisma error mapper
   * Use this in services instead of raw try/catch to convert Prisma codes to HTTP exceptions
   * Example usage: this.prisma.handlePrismaError(error)
   */
  ...
  //   if (error.code === 'P2003') throw new BadRequestException('Foreign key constraint failed');
...
async function main() {
  ...
  const user = await prisma.user.create({
    data: {
      ...
      password: 'hashed_placeholder', // replace with bcrypt hash when auth is wired
      expenses: {
        create: [
          { amount: 1200.00, category: 'Groceries',  description: 'Big Basket order',   date: new Date('2026-04-01') },
          { amount: 15000.00, category: 'Rent',       description: 'April rent',          date: new Date('2026-04-01') },
          { amount: 450.00,  category: 'Transport',  description: 'Ola rides',           date: new Date('2026-04-02') },
          { amount: 2800.00, category: 'Dining',     description: 'Team lunch + dinner', date: new Date('2026-04-03') },
          { amount: 999.00,  category: 'Subscriptions', description: 'Netflix + Spotify', date: new Date('2026-04-03') },
        ],
      },
    },
  });
...
File: backend/src/user/Backend.md
...
/** Protects routes — returns 401 if no valid JWT is present */
@Injectable()
export class JwtAuthGuard extends AuthGuard('jwt') {}
...
/** Attaches `req.user = { id, email }` on every protected route */
@Injectable()
export class JwtStrategy extends PassportStrategy(Strategy) {
...
File: backend/src/expense/dto/create-expense.dto.ts
```ts
import {
  IsDateString,
  IsNotEmpty,
...
export class CreateExpenseDto {
  /** Amount in rupees (e.g. 1200.50) */
  @IsNumber({ maxDecimalPlaces: 2 })
  @IsPositive()
  @Type(() => Number)
  amount: number;

  /** Category label e.g. "Groceries", "Rent" */
  @IsString()
  @IsNotEmpty()
  @MaxLength(100)
  category: string;
  ...
  /** ISO 8601 date string e.g. "2026-04-04" */
  @IsDateString()
  date: string;
}
...
File: backend/src/expense/expense.controller.ts
```ts
import {
  Controller,
  Post,
...
export class ExpenseController {
  ...
  async create(@Body() dto: CreateExpenseDto, @Req() req: AuthRequest) {
    return this.expenseService.createExpense(dto, req.user.id);
  }

  /** GET /expenses — list only the logged-in user's expenses */
  @Get()
  async findAll(@Req() req: AuthRequest) {
    return this.expenseService.getAllExpenses(req.user.id);
  }

  /** DELETE /expenses/:id — only the owner may delete */
  @Delete(':id')
  async remove(
...
File: backend/src/expense/expense.module.ts
```ts
import { Module } from '@nestjs/common';
import { ExpenseService } from './expense.service';
...
@Module({
  imports: [
    PrismaModule,
    AuthModule, // provides JwtAuthGuard and PassportModule
  ],
...
File: backend/src/expense/expense.service.ts
```ts
import {
  Injectable,
  NotFoundException,
...
export class ExpenseService {
  ...
  /** Create a new expense owned by the given user */
  ...
  /** Return only expenses belonging to the given user */
  ...
  /** Delete an expense — verifies ownership before deleting */
  ...
  /** Aggregated total spent by a specific user */
...
```json
{
  ...
  "prisma": {
    "seed": "ts-node --project tsconfig.json -e esm src/prisma/seed.ts"
  },
  "jest": {
...
async function bootstrap() {
  try {
    ...
    console.log(`🚀 Finance Tracker API running on: ${await app.getUrl()}`);
...
// ✅ With this (atomic + proper error handling):
async deleteUser(id: number) {
  try {
    // 🗑️ Prisma.delete throws PrismaClientKnownRequestError (P2025) if not found
    // No need for a preliminary findOne() call
    ...
    // 🚨 Convert Prisma's P2025 to a NestJS NotFoundException
    if (error.code === 'P2025') {
 ...
 * 
 * Orchestrates global configuration, cross-cutting concerns, and feature module wiring.
 * Follows NestJS modular architecture principles: loose coupling, clear boundaries, and fail-fast validation.
 */
...
import * as Joi from 'joi'; // Schema validation library for environment variables
 ...
 * Ensures all required environment variables exist and match expected formats BEFORE the app starts.
 * Prevents cryptic runtime crashes by failing fast during bootstrap.
 */
const envValidationSchema = Joi.object({
  ...
  DATABASE_URL: Joi.string().uri().required().label('Database connection string'),
  JWT_SECRET: Joi.string().min(32).required().label('JWT signing key (min 32 chars)'),
  JWT_EXPIRATION: Joi.string().default('1h'),
  CORS_ORIGINS: Joi.string().default('http://localhost:3001'),
  THROTTLE_TTL: Joi.number().default(60),   // Time window in seconds
  THROTTLE_LIMIT: Joi.number().default(10), // Max requests per TTL window
  REDIS_URL: Joi.string().uri().optional(), // Optional for distributed caching
});
...
@Module({
  imports: [
     ...
     * - isGlobal: true → ConfigService becomes injectable in ALL modules without re-importing
     * - envFilePath: loads `.env.local` first (for dev overrides), falls back to `.env`
     * - validationSchema: runs Joi validation at startup; throws if required vars are missing
     * - validationOptions: `allowUnknown: false` rejects typos in .env; `abortEarly: false` shows ALL errors
     * - cache: true → stores parsed env vars in memory for faster repeated access
     */
    ConfigModule.forRoot({
      isGlobal: true,
    ...
    /**
     * 🚦 ThrottlerModule - Global Rate Limiting Configuration
     * - Protects against brute-force, scraping, and API abuse
     * - Values injected from ConfigService so limits can differ per environment (dev vs prod)
     * - The actual guard (`ThrottlerGuard`) is applied globally in `main.ts`
     */
    ...
    /**
     * 🗄️ CacheModule - Response & Data Caching Layer
     * - Dynamic factory: switches between in-memory (dev) and Redis (prod) based on env
     * - `isGlobal: true` → `@CacheInterceptor()` and `CacheService` available everywhere
    ...
    CacheModule.registerAsync({
      ...
      useFactory: async (config: ConfigService) => {
        ...
        // Production: Redis for shared, distributed cache across instances
        if (isProduction && redisUrl) {
          const { default: redisStore } = await import('cache-manager-redis-store');
          return {
            store: redisStore,
            url: redisUrl,
            ttl: 300, // Default cache expiry: 5 minutes
          };
        }

        // Development: In-memory LRU cache (no external dependency required)
        return {
          ttl: 300,
          max: 100, // Keep max 100 items in memory
        };
      },
    }),

    /**
     * 🔗 Feature Modules - Domain-Driven Wiring
     * Order matters: Database → Authentication → Business Domains
     * Each module encapsulates its own controllers, services, DTOs, and DB access.
     */
    PrismaModule,    // 🗄️ Data access layer (handles DB connection, transactions, migrations)
    UserModule,      // 👤 User profiles, settings, CRUD operations
    AuthModule,      // 🔐 JWT strategies, login/register, password hashing, guards
    ExpenseModule,   // 💰 Core business logic (finance tracking, reports, validation)
  ],

  // 🎮 Controllers: Handle incoming HTTP requests for the root module
  // Typically used for health checks, app metadata, or global redirects
  controllers: [AppController],

  // ⚙️ Providers: Services available to other modules that import AppModule
  // Kept minimal to enforce proper module encapsulation
  providers: [AppService],

  // 📤 Exports: Make internal providers available to other modules
  // Empty here because AppModule is rarely imported by feature modules
  exports: [],
})
export class AppModule {
  /**
   * 🏗️ Constructor: Runs after module metadata is processed
   * Used here for startup diagnostics and environment confirmation
   */
  constructor(private readonly configService: ConfigService) {
    const logger = new Logger('AppModule');
    logger.log(`✅ Configuration loaded for: ${this.configService.get('NODE_ENV')} environment`);
  }
}
```
 ...
 * 
 * This file initializes the NestJS application, applies global middleware/pipes/guards,
 * configures security headers, CORS, and sets up graceful shutdown for zero-downtime deployments.
 */
...
async function bootstrap() {
  // 🛠️ Create NestJS instance with bufferLogs to prevent lost logs during startup
  const app = await NestFactory.create(AppModule, { bufferLogs: true });
  const logger = new Logger('Bootstrap');
  
  // 🔑 Retrieve ConfigService for type-safe, validated environment access
  const configService = app.get(ConfigService);

  try {
    // 🔐 SECURITY: Apply Helmet to set 14+ critical HTTP security headers
    // (Prevents XSS, clickjacking, MIME sniffing, enforces HTTPS where applicable)
    app.use(helmet());

    // 🌐 GLOBAL API PREFIX: Prefixes all routes with /api
    // Keeps versioning flexible (e.g., /api/v1, /api/v2)
    app.setGlobalPrefix('api');

    // ✅ GLOBAL VALIDATION PIPE: Auto-validates & transforms all incoming DTOs
    app.useGlobalPipes(
      new ValidationPipe({
        whitelist: true,            // 🗑️ Strips properties not defined in the DTO
        forbidNonWhitelisted: true, // 🚫 Throws 400 if unknown properties are sent
        transform: true,            // 🔄 Converts raw payloads to class instances (enables decorators)
        stopAtFirstError: true,     // ⚡ Stops validation on first error for better performance
        // Hide detailed validation errors in production to prevent schema leakage
        disableErrorMessages: configService.get('NODE_ENV') === 'production',
      }),
    );

    // 🚦 RATE LIMITING: Globally apply throttle guard (limits configured in AppModule)
    // Prevents brute-force, scraping, and API abuse
    app.useGlobalGuards(new ThrottlerGuard());

    // 🌍 CORS CONFIGURATION: Secure cross-origin requests with dynamic allowlist
    ...
    app.enableCors({
      origin: (origin, callback) => {
        ...
        // Validate origin against environment allowlist
    ...
    const host = configService.get<string>('HOST', '0.0.0.0'); // Binds to all interfaces in production

    await app.listen(port, host, () => {
      logger.log(`🚀 Finance Tracker API running on: ${await app.getUrl()}`);
      logger.log(`📦 Environment: ${configService.get('NODE_ENV', 'development')}`);
    });

    // 🔄 GRACEFUL SHUTDOWN: Ensures clean termination for Docker/K8s deployments
    // Closes DB connections, stops queues, and flushes logs before exit
    ...
    // 🚨 LOG CRITICAL STARTUP ERRORS & EXIT WITH FAILURE CODE
    logger.error(`❌ Failed to start application: ${error.message}`, error.stack);
    // Exit code 500 signals orchestrators (PM2, Docker, K8s) to restart the container
    process.exit(HttpStatus.INTERNAL_SERVER_ERROR);
  }
}

// 🏁 Execute bootstrap (never resolves in normal operation)
bootstrap();
```
